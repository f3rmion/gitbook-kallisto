...
\text{with}\quad\mathbf{v} = \mathbf{v}_{\vert\vert} + \mathbf{v}_{\perp}\\
\mathbf{v}_{\vert\vert} = (\mathbf{v}\cdot\mathbf{k})\mathbf{k} \quad(\text{projection})\\
\mathbf{v}_{\perp} = \mathbf{v} - \mathbf{v}_{\vert\vert} = \mathbf{v} - (\mathbf{k}\cdot\mathbf{v})\mathbf{k} = -\mathbf{k}\times(\mathbf{k}\times\mathbf{v})
$$
